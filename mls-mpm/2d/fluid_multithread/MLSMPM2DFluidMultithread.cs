...
public partial class MLSMPM2DFluidMultithread : Node2D
{
    struct Particle
    {
    ...
    private void P2G_2()
    {
        ...
        for (int i = 0; i < num_particles; ++i)
        {
            ...
            Transform2D viscosity_term = new Transform2D(dynamic_viscosity * strain.X, dynamic_viscosity * strain.Y, Vector2.Zero);
            stress = new Transform2D(stress.X + viscosity_term.X, stress.Y + viscosity_term.Y, Vector2.Zero);//+= viscosity_term;

            var eq_16_term_0 = new Transform2D(-volume * 4 * stress.X * dt, -volume * 4 * stress.Y * dt, Vector2.Zero);//-volume * 4 * stress * dt;
            ...
            for (gx = 0; gx < 3; ++gx)
            {
                for (gy = 0; gy < 3; ++gy)
                {
                    ...
                    Vector2 momentum = new Transform2D(eq_16_term_0.X * weight, eq_16_term_0.Y * weight, Vector2.Zero) * cell_dist; //math.mul(eq_16_term_0 * weight, cell_dist);
                    cell.vel += momentum;
    ...
    private void UpdateCell(int i)
    {
        ...
        if (cell.mass > 0)
        {
            ...
            // stick
            //if (x < 2 || x > grid_res - 3) { cell.vel = new Vector2(0.0f, 0.0f); }
            //if (y < 2 || y > grid_res - 3) { cell.vel = new Vector2(0.0f, 0.0f); }

            // slip + friction
            float friction = 0.5f; // 0 to 1
            if (x < 2 || x > grid_res - 4) { cell.vel = new Vector2(0.0f, friction * cell.vel.Y); }
            if (y < 2 || y > grid_res - 4) { cell.vel = new Vector2(friction * cell.vel.X, 0.0f); }
    ...
    private void G2PSingle(int i)
    {
        ...
        Vector2 cell_diff = (p.pos - cell_idx) - new Vector2(0.5f, 0.5f);
        Vector2[] weights = {
                0.5f * ((new Vector2(0.5f, 0.5f) - cell_diff) * (new Vector2(0.5f, 0.5f) - cell_diff)),
                new Vector2(0.75f, 0.75f) - (cell_diff * cell_diff),
                0.5f * ((new Vector2(0.5f, 0.5f) + cell_diff) * (new Vector2(0.5f, 0.5f) + cell_diff))
        };
        ...
        p.pos = p.pos.Clamp(1, grid_res - 1);
        ...
        // predictive particle damping
        Vector2 x_n = p.pos + p.vel;
        Vector2 damping_factor = new Vector2(0.5f, 0.5f);
        int wall_min = 2;
        int wall_max = grid_res - 1 - wall_min;
        if (x_n.X < wall_min) p.vel.X += damping_factor.X * (wall_min - x_n.X);
        if (x_n.X > wall_max) p.vel.X += damping_factor.X * (wall_max - x_n.X);
        if (x_n.Y < wall_min) p.vel.Y += damping_factor.Y * (wall_min - x_n.Y);
        if (x_n.Y > wall_max) p.vel.Y += damping_factor.Y * (wall_max - x_n.Y);
